...
public class CxcActService : ICxcActService
{
    ...
    )
    {
        try
        {
            ...
            if (!deleted)
                return new ApiResponse<bool>(false, "CXC actual no encontrado")
                {
                    StatusCode = 404,
                };

            return new ApiResponse<bool>(true, "CXC actual eliminado exitosamente", true)
            {
                StatusCode = 200,
            };
        }
        catch (DbUpdateException ex)
        {
            _logger.LogWarning(
                ex,
                "Error de BD al eliminar CXC actual {CiaCodigo}/{OfiCodigo}/{MonCodigo}/{CcaModulo}/{CcaTipo}/{CcaCodigo}/{CcaAno}/{CcaNumero}/{CcaSecuencia}/{CcaSubsec}",
                ciaCodigo,
                ofiCodigo,
                monCodigo,
                ccaModulo,
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error al eliminar CXC actual {CiaCodigo}/{OfiCodigo}/{MonCodigo}/{CcaModulo}/{CcaTipo}/{CcaCodigo}/{CcaAno}/{CcaNumero}/{CcaSecuencia}/{CcaSubsec}",
                ciaCodigo,
                ofiCodigo,
                monCodigo,
                ccaModulo,
