...
public class GrpoCteService : IGrpoCteService
{
    ...
    public async Task<ApiResponse<bool>> DeleteAsync(int ciaCodigo, int gctCodigo)
    {
        try
        {
            var deleted = await _repository.DeleteAsync(ciaCodigo, gctCodigo);
            if (!deleted)
                return new ApiResponse<bool>(false, "Grupo de cliente no encontrado")
                {
                    StatusCode = 404,
                };

            return new ApiResponse<bool>(true, "Grupo de cliente eliminado exitosamente", true)
            {
                StatusCode = 200,
            };
        }
        catch (DbUpdateException ex)
        {
            _logger.LogWarning(
                ex,
                "Error de BD al eliminar grupo de cliente {Compania}/{Codigo}",
                ciaCodigo,
                gctCodigo
            );
            return new ApiResponse<bool>(
                false,
                "No se puede eliminar: existen clientes asociados a este grupo"
            )
            {
                StatusCode = 409,
                Errors = [ex.InnerException?.Message ?? ex.Message],
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error al eliminar grupo de cliente {Compania}/{Codigo}",
                ciaCodigo,
                gctCodigo
            );
