...
public class ClienteService : IClienteService
{
    ...
    public async Task<ApiResponse<bool>> DeleteAsync(int ciaCodigo, string cteCodigo)
    {
        try
        {
            var deleted = await _repository.DeleteAsync(ciaCodigo, cteCodigo);
            if (!deleted)
                return new ApiResponse<bool>(false, "Cliente no encontrado") { StatusCode = 404 };

            return new ApiResponse<bool>(true, "Cliente eliminado exitosamente", true)
            {
                StatusCode = 200,
            };
        }
        catch (DbUpdateException ex)
        {
            _logger.LogWarning(
                ex,
                "Error de BD al eliminar cliente {Compania}/{Codigo}",
                ciaCodigo,
                cteCodigo
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error al eliminar cliente {Compania}/{Codigo}",
                ciaCodigo,
                cteCodigo
            );
